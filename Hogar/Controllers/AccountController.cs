...
namespace Hogar.Controllers
{
    public class AccountController : Controller
    {
        ...
        public AccountController(IServiceUsuario usuarioService, HogarContext _context)
        {
        ...
        public async Task<IActionResult> Login(string username, string password)
        {
            ...
            if (user != null)
            {
                ...
                if (user.Estado == 0) // Asegúrate de que la propiedad sea `Estado` o cambia según tu modelo
                {
        ...
        private async Task<bool> EnviarCorreoRecuperacion(string correo, string nuevaContraseña)
        {
            try
            {
                ...
                {
                    Port = 587,
                    Credentials = new NetworkCredential("noreplyhogarjafethjimenez@gmail.com", "gvbh ueos xzyy pbmm"),
                    EnableSsl = true
                };

                var mensaje = new MailMessage
                {
                    From = new MailAddress("noreplyhogarjafethjimenez@gmail.com"),
